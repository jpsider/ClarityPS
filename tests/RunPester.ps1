...
foreach ($Directory in $Directories)
{
    ...
    $files = Get-ChildItem .\ClarityPS\$Directory
    foreach ($file in $files)
    {
        # Source the file
        $FileName = $file.Name
        Write-Output "Importing file $FileName"
        . .\ClarityPS\$Directory\$FileName
    }
    # Execute Pester for the Directory.
    $results = Invoke-pester .\tests\$Directory\*.ps1 -CodeCoverage .\ClarityPS\$Directory\*.ps1 -PassThru -ExcludeTag SkipThis
